...
const YearsOfExperienceRoleSelectionV4 = () => {
  ...
  const handleCheckboxChange = (e) => {
    ...
    update({
      type: URLSearchStateUpdateType.EXCLUDE_IF_ROLE_YOE_IS_NOT_SPECIFIED,
      payload: e.target.checked,
    });
  };

  useEffect(() => {
    setRange(currentSearchState.roleYoeRange);
    setExcludeNoYoe(currentSearchState.excludeIfRoleYoeIsNotSpecified);
  }, [
    currentSearchState.roleYoeRange,
    currentSearchState.excludeIfRoleYoeIsNotSpecified,
  ]);
      ...
      >
        <div className="flex items-center space-x-2 mb-4">
          ...
          {(range[0] > 0 || range[1] < maxYearsOfExperience) && (
            <button
              onClick={() => {
                update({
                  type: URLSearchStateUpdateType.ROLE_YOE_RANGE,
        ...
        {(range[0] > 0 || range[1] < maxYearsOfExperience) && (
          <Flex align="center">
            <Checkbox
              colorScheme="pink"
              id="yoe_role_checkbox"
              isChecked={excludeNoYoe}
              onChange={handleCheckboxChange}
            />
            <div className="flex items-center space-x-2">
              <label htmlFor="yoe_role_checkbox" className="ml-2 text-sm">
              ...
              <Popover>
                ...
                <PopoverContent>
                  <PopoverArrow />
                  <PopoverBody p={4}>
                    <span className="text-sm">{`By default, jobs without specified years of role / industry experience are included in the search results. Selecting this checkbox will exclude those jobs, ensuring only positions that meet your specified criteria are shown.`}</span>
                  </PopoverBody>
                </PopoverContent>
              </Popover>
            </div>
...
export default YearsOfExperienceRoleSelectionV4;
