...
const OutroStep = ({ screenID, job_id, public_access = null }) => {
  const { screen } = useScreenData(screenID);
  const [outro, setOutro] = useState(null);

  useEffect(() => {
    if (screen && screen.outro) {
      setOutro(screen.outro);
    }
  }, [screen]);
  ...
  const handleUpdateOutro = () => {
    window.videoask.loadModal({
      url: `https://ask.hiring.cafe/fwd7p89y6#question_to_update=${screen.outro.videoask_question_id}&screen_id=${screenID}&screen_type=INITIAL_VIDEO_SCREEN_OUTRO`, // TODO: Make videoask url dynamic
      options: { modalType: "Fullscreen" },
    });
  };

  if (!outro || !screen) return null;
        ...
        {outro.media_url && (
          <div className="flex flex-col space-y-10 w-full">
            <span className="font-medium">
              {
                "Thank the candidates and let them know that your team will reach out to them for next steps if they're a good fit."
              }
            </span>
            <VideoPlayer videoSrcs={[outro.media_url]} showControls={true} />
            <div>
              {outro.is_processing_video_update ? (
                <div>
